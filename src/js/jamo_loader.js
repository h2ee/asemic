...
function remapPos(pos, from, to) {
  if (!pos || pos.every((v) => v === null || v === undefined || isNaN(v)))
    return null;
  return pos.map(
    (v) => to[0] + ((v - from[0]) / (from[1] - from[0])) * (to[1] - to[0])
  );
}
...
export async function loadJamo(options = {}) {
  const {
    csvPath = "./src/assets/tables/jamo_data.csv",
  ...
  for (const row of rows) {
    ...
    const rawPos = hasPos
      ? [parseFloat(row.x), parseFloat(row.y), parseFloat(row.z)]
      : null;
...
/** 이중모음 */
export const DIPHTHONG = new Set([
  "ㅑ",
  "ㅒ",
  "ㅕ",
  "ㅖ",
